...
namespace WebSocketExtensions
{
    ...
    public static class HelperExtensions
    {
        ...
        public static async Task SendStreamAsync(this WebSocket ws, Stream stream, bool dispose = false, CancellationToken tok =  default(CancellationToken))
        {
            try
            {
                ...
                while (remaining > 0)
                {
                    ...
                    await ws.SendAsync(new ArraySegment<byte>(buffer), WebSocketMessageType.Binary, isLast, tok);
        ...
        {
            return ws.SendAsync(new ArraySegment<byte>(data), WebSocketMessageType.Binary, true, tok);

        }

        public static Task SendStringAsync(this WebSocket ws, string data, CancellationToken tok = default(CancellationToken))
        {
            return ws.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(data)), WebSocketMessageType.Text, true, tok);
